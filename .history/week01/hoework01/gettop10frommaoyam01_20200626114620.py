...
def get_urls(url, headers):
    rresponse = requests.get(maoyanUrl,headers=header)   
...
for tag in bs_info.find_all('div',):
    for tag_p in tag.find_all('a',href=re.compile('/films/')) :
        # 打印名称
        print(tag_p.get('href'))
        films_url.append(maoyanUrl + tag_p.get('href'))

urls = set(films_url)
print(urls)
pesponse = requests.get(maoyanUrl,headers=header)   
# response.encoding = 'utf-8' 
bs_info = bs(response.text,"html.parser")
