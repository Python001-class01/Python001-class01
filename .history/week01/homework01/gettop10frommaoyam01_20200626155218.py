...
header = {
    ...
    'Cookie' : '__mta=251934006.1593072991075.1593155308242.1593155851862.24; uuid_n_v=v1; uuid=2395D3F0B6BC11EA9F28E30FF5FFF73C9A16AE2FA53A448DA75AEAA9D715CB59; _csrf=8557626db9b655cf9050ae7e5b2aab69278c8061c21eca95e1c3cf2130b0b64c; _lxsdk_cuid=172ea8cb247c8-0a73066b1c0a8b-4353760-100200-172ea8cb248c8; _lxsdk=2395D3F0B6BC11EA9F28E30FF5FFF73C9A16AE2FA53A448DA75AEAA9D715CB59; mojo-uuid=c457eacb7c1eb59d3d2f6c1f8d75b9c9; Hm_lvt_703e94591e87be68cc8da0da7cbd0be2=1593072989,1593073002; _lx_utm=utm_source%3Dgoogle%26utm_medium%3Dorganic; __mta=251934006.1593072991075.1593140975947.1593145813576.21; mojo-session-id={"id":"fdbb8b9a182fe9a2d581ce714af0b226","time":1593155308059}; mojo-trace-id=2; Hm_lpvt_703e94591e87be68cc8da0da7cbd0be2=1593155852; _lxsdk_s=172ef74b842-843-bf-29f%7C%7C5',
...
def get_urls(url, headers):
    ...
    for tag in bs_info.find_all('div',):
        # 获取top10电影详情页链接
        hrefs = tag.find_all('dd',limit=10)
        for ts in hrefs:
            films_url.append(main_url + ts.find('a',).get('href'))

    urls = list(set(films_url))
    urls.sort(key=films_url.index)
    print(urls)
    return urls
...
def save_movies(movies):
    print(movies)
    top10 = pd.DataFrame(data=movies,columns=['电影名称','电影类型','s'])
